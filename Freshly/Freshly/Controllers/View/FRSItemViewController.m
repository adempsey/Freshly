...
@interface FRSItemViewController ()
...
@property (nonatomic, readwrite, strong) FRSItemDateViewController *itemDateViewController;
...
@property (nonatomic, readwrite, strong) FRSFoodAutoCompletionViewController *autoCompletionViewController;
...
@implementation FRSItemViewController
...
- (instancetype)initWithItem:(FRSFoodItem *)item
{
	if (self = [super init]) {
		...
		self.itemDateViewController = [[FRSItemDateViewController alloc] init];
		...
		self.categoryList = [[NSArray alloc] initWithArray:[[FRSFoodItemService sharedInstance] foodItemCategoryList]];
		...
		self.autoCompletionViewController = [[FRSFoodAutoCompletionViewController alloc] init];
...
- (void)viewDidLoad
{
	...
	UIColor *categoryColor = [[FRSFoodItemService sharedInstance] colorForCategory:self.item.category];
...
- (void)viewDidDisappear:(BOOL)animated
{
	...
	if (self.itemExists) {
		...
		[[FRSFoodItemService sharedInstance] updateItem:self.item];
	}
}
...
- (void)deleteItem
{
	[[FRSFoodItemService sharedInstance] deleteItem:self.item];
	self.itemExists = NO;
...
- (void)saveNewItem
{
	...
	[[FRSFoodItemService sharedInstance] createItemWithAttributes:attributes];
	...
	[UIView animateWithDuration:0.25 animations:^{
		...
		if (self.currentPicker) {
			...
			if ([self.currentPicker isKindOfClass:[UIPickerView class]] && [self.currentPicker isEqual:self.categoryPicker]) {
				...
				UIColor *categoryColor = [[FRSFoodItemService sharedInstance] colorForCategory:categoryTitle];
				...
				if ([self.currentPicker isKindOfClass:[UIDatePicker class]]) {
					...
					if ([self.currentPicker isEqual:self.purchaseDatePicker]) {
						...
						if (!self.userUpdatedExpirationDate) {
							NSString *selectedSpace = [[FRSFoodItemService sharedInstance] titleForSpaceIndex:self.spaceChooser.selectedSegmentIndex];
							NSInteger defaultExpirationTime = [[FRSFoodItemService sharedInstance] defaultExpirationTimeForFoodItemName:self.titleField.text inSpace:selectedSpace];
...
- (void)didSelectAutoCompletedItem:(NSString *)item withCategory:(NSString *)category
{
	...
	NSString *defaultItemSpace = [[FRSFoodItemService sharedInstance] defaultSpaceForFoodItemName:item];
	self.spaceChooser.selectedSegmentIndex = [[FRSFoodItemService sharedInstance] spaceIndexForTitle:defaultItemSpace];

	NSInteger defaultExpirationTime = [[FRSFoodItemService sharedInstance] defaultExpirationTimeForFoodItemName:item inSpace:defaultItemSpace];
	...
	UIColor *categoryColor = [[FRSFoodItemService sharedInstance] colorForCategory:category];
