...
@interface FRSShoppingListViewController ()
...
@implementation FRSShoppingListViewController
...
- (id)init
{
    ...
    if (self) {
		...
		[[FRSFoodItemService sharedInstance] retrieveItemsForShoppingListWithBlock:^(NSArray *items) {
			self.items = items;
		}];
...
- (void)viewDidAppear:(BOOL)animated
{
	...
	[[FRSSettingsService sharedInstance] setSelectedSection:1];
}
...
- (void)moveSelectedItemsToStorage
{
	...
	for (FRSFoodItem *item in checkedItemsCopy) {
		item.inStorage = [NSNumber numberWithBool:YES];

		NSString *itemSpace = [[FRSFoodItemService sharedInstance] defaultSpaceForFoodItemName:item.name];
		item.space = [NSNumber numberWithInteger:[[FRSFoodItemService sharedInstance] spaceIndexForTitle:itemSpace]];

		item.dateOfPurchase = [NSDate date];

		NSInteger expirationTime = [[FRSFoodItemService sharedInstance] defaultExpirationTimeForFoodItemName:item.name inSpace:itemSpace];
		item.dateOfExpiration = [NSDate dateWithTimeIntervalSinceNow:60*60*24*expirationTime];

		[[FRSFoodItemService sharedInstance] updateItem:item];
	}
...
{
	FRSFoodItem *item = self.items[indexPath.row];
	FRSShoppingListTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:TABLE_VIEW_CELL_SHOPPING_LIST_IDENTIFIER];
	
	if (!cell) {
		cell = [[FRSShoppingListTableViewCell alloc] initWithItem:item];
		cell.delegate = self;
	} else {
		[cell setItem:item];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
	if (editingStyle == UITableViewCellEditingStyleDelete) {
		[[FRSFoodItemService sharedInstance] deleteItem:self.items[indexPath.row]];
	}
}
...
- (void)shoppingListCell:(FRSShoppingListTableViewCell *)cell didChangeCheckboxValue:(BOOL)value
{
	...
	__weak FRSFoodItem *changedItem = self.items[cellIndex.row];
